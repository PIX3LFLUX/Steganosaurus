...
def stego_path_generator(cover_img_path: str, img_type: ImageType):
    ...
    stegoname = os.path.splitext(cover_img_path)[0]
    filename= os.path.basename(stegoname)
    print("FILENAME",filename)
    stegoname= "steg"+ filename + "."+img_type.name
...
def steg_decode_simple(stego_img_path: str, cut: float=None) -> str:

    set_img_path(cover_img_path, stego_img_path)
    # set_colorspace(colorspace)
    try:
        text = steg_decode(cut)[0]
    except UnicodeDecodeError:
